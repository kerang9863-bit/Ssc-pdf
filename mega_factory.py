...
# --- CONFIG ---
# Multiple providers for unlimited free tier rotation
PROVIDERS = []
...
# OpenRouter (50 RPD free per key)
...
# Gemini Direct API (1,000 RPD free)
...
def fetch_content(subject: str, prompt_detail: str, provider_idx: int, retries: int = 3) -> Tuple[Optional[str], bool]:
    ...
    full_prompt = (
        ...
        "1. Every question MUST start with Q1. , Q2. , Q3. etc.\n"
        "2. Immediately after EACH question, write Ans: followed by the answer\n"
        "3. Example format:\n"
        "   Q1. What is the capital of France?\n"
        "   Ans: Paris\n"
        "   Q2. [Next question]\n"
        "   Ans: [Answer]\n"
        "4. For Math: After Ans:, add Short Trick: then Solution:\n"
        "5. NO markdown, NO **, NO tables, NO placeholder notes\n"
        "6. Only use established facts, no speculation about future events"
    )
...
def create_pdf(content: str, subject: str) -> str:
    """Bulletproof PDF generation with latin-1 safe encoding."""
    pdf = FPDF()
    pdf.add_page()
    pdf.set_auto_page_break(auto=True, margin=15)

    # Use standard Arial font - no Unicode complications
    font_name = "Arial"
    ...
    for raw_line in content.split('\n'):
        ...
        is_answer = any(x in line.lower() for x in [
            'ans:', 'answer:', 'solution:', 'short trick:', 
            'explanation:', 'correct answer:', 'step',
            'division:', 'multiplication:', 'addition:', 'subtraction:'
        ])

        # Numbered steps like "1. Division: 5/10 = 0.5"
        is_step = re.match(r'^\d+[\.\)]\s+[A-Za-z]', line) and any(x in line.lower() for x in ['division', 'multiplication', 'addition', 'subtraction', 'step', 'calculation'])
        if is_step:
            is_answer = True
        ...
        # CRITICAL FIX: Pre-encode to latin-1 to prevent render errors
        # Replace any non-latin1 character with ? to avoid "not enough space" crashes
        safe_line = line.encode('latin-1', 'replace').decode('latin-1')

        # Use fixed width to prevent "not enough horizontal space" error
        try:
            # w=0 means full width, but we use explicit width to be safe
            available_width = pdf.w - pdf.l_margin - pdf.r_margin
            pdf.multi_cell(available_width, 5, safe_line)
        except Exception as e:
            log(f"PDF render error (skipping line): {e}")
            # Try with even safer fallback - truncate and add ...
            try:
                truncated = safe_line[:80] + "..." if len(safe_line) > 80 else safe_line
                pdf.multi_cell(available_width, 5, truncated)
            except:
                log(f"Fallback render also failed, skipping entirely")
                continue
