...
def create_pdf(content: str, subject: str) -> str:
    ...
    try:
        ...
        if os.path.exists(dejavu_path):
            ...
            font_name = "Arial"
            log("DejaVu not found, using Arial")
    except Exception as e:
        font_name = "Arial"
    ...
    for line in content.split('\n'):
        ...
        # Expanded answer detection
        is_answer = any(x in line.lower() for x in [
            'ans:', 'answer:', 'solution:', 'short trick:', 
            'explanation:', 'correct answer:', 'step', 'division:', 
            'multiplication:', 'addition:', 'subtraction:'
        ])

        # Check if line starts with a numbered step (like "1. Division: 5/10 = 0.5")
        is_numbered_step = re.match(r'^\d+[\.\)]\s+[A-Za-z]', line)
        if is_numbered_step and any(x in line.lower() for x in ['division', 'multiplication', 'addition', 'subtraction', 'step']):
            is_answer = True

        is_question = re.match(r'^Q?\d+[\.\)\:]', line) or line.lower().startswith('question')

        if is_answer:
            pdf.set_text_color(0, 128, 0)  # Green for answers
            pdf.set_font(font_name, 'B', 10)
        elif is_question:
            pdf.set_text_color(0, 0, 0)  # Black for questions
            pdf.set_font(font_name, 'B', 10)
            pdf.ln(2)
        else:
            pdf.set_text_color(50, 50, 50)  # Gray for other text
            pdf.set_font(font_name, '', 10)
...
if __name__ == "__main__":
    ...
    log(f"Starting SSC Factory with {len(API_KEYS)} API keys")
